...
def check_hull_params(params: HullBasicParams) -> ParamCheckResult:
    ...
    if not all(field.strip() for field in required_fields if field is not None): # type: ignore
    ...
    try:
        ...
        loa = float(params.Loa) if (params.Loa and params.Loa.strip()) else None # type: ignore
...
class HullParamInputWindow(QMainWindow, Ui_MainWindow):
    ...
    def init_ui_state(self):
        ...
        # 状态栏初始提示
        self.update_status_message("请输入船体基本参数(带*为必填项)")
    ...
    def set_input_filters(self):
        ...
        float_validator = QRegExpValidator(float_regex, self)

        # 为所有必填参数输入框应用过滤器
        input_edits = [self.le_lpp, self.le_b, self.le_d, self.le_t, self.le_delta]
        for edit in input_edits:
            edit.setValidator(float_validator)
            edit.setAlignment(Qt.AlignRight)   # type: ignore # 数字输入右对齐(符合用户习惯)
    ...
    def handle_confirm_input(self):
        ...
        if check_result.is_valid and check_result.valid_params is not None:
            ...
            # 状态栏提示
            self.update_status_message(f"✅ {check_result.error_msg},可进行后续建模")
            # 弹出成功对话框(展示详细参数)
            ...
            # 校验失败:显示错误信息(任务单元1要求)
            self.update_status_message("❌ 参数校验失败,请修正输入")
